...
export const Route = createFileRoute("/estoque")({
  head: () => ({
    meta: [
      { title: "Estoque — Costa Veículos" },
      { name: "description", content: "Confira nosso estoque completo de carros e motos seminovos com garantia e procedência." },
      { property: "og:title", content: "Estoque de seminovos — Costa Veículos" },
    ],
  }),
  component: Estoque,
});
...
function Estoque() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("vehicles")
        .select("id,brand,model,year,mileage,price,fuel,status,images,category,created_at")
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.sort === "price-asc") out = [...out].sort((a, b) => Number(a.price) - Number(b.price));
    if (filters.sort === "price-desc") out = [...out].sort((a, b) => Number(b.price) - Number(a.price));
    ...
    <div className="mx-auto max-w-7xl px-4 md:px-8 pt-16 pb-24">
      ...
      <div className="grid lg:grid-cols-[280px_1fr] gap-8">
        <aside className={`${openMobile ? "fixed inset-0 z-50 bg-background p-6 overflow-auto" : "hidden"} lg:block lg:static lg:bg-transparent lg:p-0`}>
          {openMobile && (
            <button onClick={() => setOpenMobile(false)} className="lg:hidden absolute top-4 right-4 p-2"><X className="size-5" /></button>
          )}
...
function FiltersPanel({ filters, setFilters }: { filters: Filters; setFilters: (f: Filters) => void }) {
  ...
  return (
    <div className="space-y-6 rounded-xl border border-border/60 bg-card/40 p-5">
      ...
      <div>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          {[{ v: "all", l: "Todos" }, { v: "carro", l: "Carros" }, { v: "moto", l: "Motos" }].map((o) => (
            <button key={o.v} onClick={() => set("category", o.v)} className={`rounded-md border px-2 py-2 text-xs uppercase tracking-wider transition ${filters.category === o.v ? "border-silver bg-silver text-black" : "border-border hover:border-silver/40"}`}>
              {o.l}
            </button>
          ))}
        </div>
      ...
      <button onClick={() => setFilters(EMPTY)} className="w-full rounded-md border border-border py-2 text-xs uppercase tracking-widest hover:border-silver/40">
        Limpar filtros
      </button>
    </div>
  );
