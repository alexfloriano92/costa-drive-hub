...
export function VehicleCard({ v }: { v: VehicleCardData }) {
  ...
  return (
    <Link
      to="/veiculo/$id"
      params={{ id: v.id }}
      className="group relative flex flex-col overflow-hidden rounded-xl border border-border/60 bg-card transition-all hover:border-silver/40 hover:shadow-premium"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        <SignedImage path={v.images?.[0]} alt={`${v.brand} ${v.model}`} className="size-full object-cover transition-transform duration-700 group-hover:scale-105" />
        {sold && (
