...
export default function Veiculo() {
  ...
  const { data: v, isLoading, isError } = useQuery({ queryKey: ["vehicle", id], queryFn: () => api.getVehicle(id), enabled: !!id });
  ...
  const msg = `Olá! Tenho interesse no ${v.brand} ${v.model} ${v.year} anunciado por ${formatBRL(v.price)}.`;
  const specs = [
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_400px]">
        <div>
          <div className="relative aspect-[4/3] overflow-hidden rounded-xl border border-border bg-card">
            <VehicleImage path={v.images?.[activeImg]} alt={`${v.brand} ${v.model}`} loading="eager" className="size-full object-cover" />
            {sold && (
        ...
        <aside className="space-y-6">
          ...
          <div className="rounded-xl border border-silver/20 bg-card p-6 shadow-premium">
            ...
            <div className="mt-1 font-display text-4xl text-gradient-silver">{formatBRL(v.price)}</div>
            {!sold && (
